...
func platformer_controls(input_vector: Vector2) -> void:
	...
	match Global.MAIN.level_data.look_type:
		Util.LOOK_TYPES.MOUSE:
			if get_global_mouse_position().x < global_position.x:
				look_dir = -1
			elif get_global_mouse_position().x > global_position.x:
				look_dir = 1
		Util.LOOK_TYPES.MOVEMENT or Util.LOOK_TYPES.NONE:
			if input_vector.x != 0:
				@warning_ignore("narrowing_conversion")
				look_dir = input_vector.x
	...
	if is_on_floor():
		...
		if Input.is_action_just_pressed("JUMP"):
			# TODO: Replace with jump animation that sets y velocity to jumpvel once finished
			velocity.y = JUMPVEL
